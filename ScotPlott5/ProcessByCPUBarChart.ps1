...
# Check version: ([ScottPlot.Plot]).Assembly.GetName().Version

$SampleData = Get-Process | Sort-Object CPU -Descending | Select-Object -First 15 -Unique
...
# Set titles
$Plot.Title("Process by CPU usage")
$Plot.YLabel("CPU(s)")
#$Plot.XLabel("X-Axis")
...
$ticks = [ScottPlot.Tick[]]@(
    for ($i = 0; $i -lt $SampleData.Count; $i++) {
        [ScottPlot.Tick]::new($i, "$($SampleData[$i].ProcessName)")
    }
)
$Bars = New-Object 'System.Collections.Generic.List[ScottPlot.Bar]'
for ($i = 0; $i -lt $SampleData.Count; $i++) {    
    $Bar = [ScottPlot.Bar]::new()
    $Bar.Position = $i
    $Bar.Value = $SampleData[$i].CPU
    $Bars.Add($Bar)
}
